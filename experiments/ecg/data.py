...
def load_data(opt):
    ...
    if opt.dataset =="ecg":
        N_samples = np.load(os.path.join(opt.dataroot, "N_samples.npy"))
        ...
        # Test / Train split
        test_N, test_N_y, train_N, train_N_y = getFolderK(N_samples, opt.folder, 0)
        ...
        test_V, val_V, test_V_y, val_V_y = getPercent(test_S, test_S_y, 0.1, 0)
        test_F, val_F, test_F_y, val_F_y = getPercent(test_S, test_S_y, 0.1, 0)
        test_Q, val_Q, test_Q_y, val_Q_y = getPercent(test_S, test_S_y, 0.1, 0)
        
        
        val_data = np.concatenate([val_N, val_S, val_V, val_F, val_Q])
        val_y = np.concatenate([val_N_y, val_S_y, val_F_y, val_Q_y])
        ...
        if not opt.istest and opt.n_aug >0:
            ...
            print('After Augmentation, Train data size: {}'.format(train_N.shape))
...
def getFolderK(data, folder, label):
