...
class Generator(nn.Module):
    def __init__(self, opt):
        ...
        self.decoder = Decoder(self.ngpu, opt)
        
    def forward(self, x):
        latent_z = self.encoder1(x) 
        gen_x = self.decoder(latent_z) 
        
        return gen_x, latent_z
...
class BeatGAN(AD_MODEL):
    ...
    def __init__(self, opt, dataloader, device):
        super().__init__(opt, dataloader, device)
        ...
        self.input = torch.empty(size=(self.opt.batchsize, self.opt.nc, self.opt.issize), dtype= torch.float32, device =self.device)
        self.label = torch.empty(size=(self.opt.batchsize,), dtype=torch.float32, device= self.device) 
        
        self.gt = torch.empty(size=(self.opt.batch_size,), dtype=torch.long, device = self.device) 
        self.fixed_input = torch.empty(size=(self.opt.batchsize, self.opt.nc, self.opt.issize), dtype=torch.float32, device= self.device)
    ...
    def train(self):
        ...
        with open(os.path.join(self.outf, self.model, self.dataset, 'val_info.txt')) as f:
    ...
    def update_netd(self):
        ...
        self.fake, self.latent_z = self.G(self.input) 
    ...
    def predict(self, dataloader_, scale=True):
        with torch.no_grad():
            ...
            self.latent_z = torch.zeros(size=(len(dataloader_.dataset), self.opt.nz), dtype=torch.float32, device= self.device)
            ...
            for i, data in enumerate(dataloader_, 0):
                self.set_input(data)
                self.fake, latent_z = self.G(self.input) 
                
                
                error = torch.mean(
                    torch.pow((self.input.view(self.input.shape[0], -1) - self.fake.view(self.fake.shape[0], -1)), 2), dim=1
                )
                
                self.an_scores[i*self.opt.batchsize : i*self.opt.batchsize+error.size(0)] = error.reshape(error.size(0))
                self.gt_labels[i*self.opt.batchsize : i*self.opt.batchsize+error.size(0)] = self.gt.reshape(error.size(0))
                self.latent_z[i*self.opt.batchsize : i*self.opt.batchsize+error.size(0), :] = latent_z.reshape(error.size(0), self.opt.nz)
    ...
    def predict_for_right(self, dataloader_, min_score, max_score, threshold, save_dir):
        ...
        with torch.no_grad():
            test_pair = []
            self.an_scores = torch.zeros(size=(len(dataloader_.dataset),), dtype= torch.float32, device= torch.device)
            
            for i, data in enumerate(dataloader_, 0):
                
                self.set_input(data)
                self.fake, latent_z = self.G(self.input) # self.input에 data의 정보로 저장
